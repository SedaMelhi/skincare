  ...
  const addCheckboxFilter = (i: string) => {
    if (checkboxFilters[id]) {
      let params = checkboxFilters[id].some((item: any) => item === i)
        ? {
            ...checkboxFilters,
            [id]: checkboxFilters[id].filter((item: any) => item !== i),
          }
        : { ...checkboxFilters, [id]: [...checkboxFilters[id], i] };
      if (params[id].length === 0) {
        delete params[id];
      }
      dispatch(setCheckboxFilters(params));
    } else {
      dispatch(setCheckboxFilters({ ...checkboxFilters, [id]: [i] }));
    }
  };
  return (
    <div className={"catalogFilter " + (firstLoad ? style.load : "")}>
      <Accordion className={"accordion"}>
        <AccordionSummary
          className={"params__wrap"}
          onClick={() => {
        ...
        <AccordionDetails className="params">
          {renderItems.map((item, i) => (
            <label className={style.item} key={item.id}>
              <input
                type="checkbox"
                name={item.id}
                value={item.name}
                checked={
                  checkboxFilters[id]
                    ? checkboxFilters[id].some(
                        (filter: any) => filter === item.id
                      )
                    : false
                }

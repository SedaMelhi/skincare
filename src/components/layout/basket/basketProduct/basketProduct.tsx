...
const BasketProduct: FC<IbasketData> = ({
  ...
  const [uniqueScuColor, setUniqueScuColor] = useState<(IShade | false)[]>(
    parentItem?.SCU.map((item) => {
      if (
        item.shade &&
        !uniqueColors[item.shade.NAME] &&
        activeScu[0].value === item.value
      ) {
    ...
    data.then((res) => {
      if (res.status === "ok") {
        ...
        data.then((res: any) => {
          ...
          setBasketArr &&
            setBasketArr(res.cartItems ? Object.values(res.cartItems) : []);
        });
      }
    });
  };
  const handleSizeClick = (newValue: string, cartId: number) => {
    addSCUToCartService.addSCUToCart(
      parentItem.SCU.filter(
        ({ value, shade }) =>
          shade?.ID === activeScu[0].shade?.ID && newValue == value
      )[0].id,
      quantity
    );
          ...
          <div className={style.flex}>
                      ...
                      <div className={style.color} key={item.ID}>
                        ...
                        <div
                          className={
                            style.color__border +
                            " " +
                            (item.ID === activeScu[0].shade?.ID
                              ? style.active
                              : "")
                          }
                          onClick={() =>
                            handleChangeActiveColor(item.ID, cartId)
                          }
                        >
                          <div
                ...
                {uniqueScuValue.map((value, i) => (
                  <div
                    className={
                      style.size +
                      " " +
                      (activeScu[0].value === value ? style.active : "")
                    }
                    key={i}
            ...
            <div>
              ...
              <div
                ...
                onClick={() => handleRemoveScuToCart(cartId)}
              >
